...
def train(rank, args, shared_model, optimizer):
    ...
    # NOTE: the original code zeroed the shared step/episode counters here in
    # every worker's first iteration, racing workers that were already
    # counting; the parent initializes them to 0 before spawning, so don't.
    ...
    while True:
        ...
        for i in reversed(range(len(player.rewards))):
            R = args.gamma * R + float(player.rewards[i])
        ...
        # set_to_none=False: ensure_shared_grads hands the local .grad tensors
        # to the shared model; freeing them each step (torch>=2 default) would
        # leave the shared model training on stale gradients
        player.model.zero_grad(set_to_none=False)
